...
class UserInterface:
  ...
  def update_content(self):
    ...
    self.content.addnstr(
      1, 2, f"{str(elapsed_delta)}/{str(duration_delta)}", max_x)
    if y > 12:
      self.content.addnstr(3, 2, f"Completed:", max_x)
      self.content.addnstr(
        4, 2, f"Work stints:  {self.engine.work_count}", max_x)
      self.content.addnstr(
        5, 2, f"Short breaks: {self.engine.short_count}", max_x)
      self.content.addnstr(
        6, 2, f"Long breaks:  {self.engine.long_count}", max_x)
      self.content.addnstr(
        8, 2, f"Total time spent:   {str(total_delta)}", max_x)
      self.content.addnstr(
        9, 2, f"Time spent working: {str(working_delta)}", max_x)

      if self.engine.started_at != None:
        start_time = time.strftime("%H:%M:%S", self.engine.started_at)
        self.content.addnstr(
          10, 2, f"First timer started at {start_time}", max_x)
  ...
  def update_sidebar(self):
    ...
    for idx, timer in enumerate(self.engine.timers):
      if idx == self.engine.current_timer_id:
        self.sidebar.addstr(
          idx+4, 2, f"{timer[0]} ({timer[1]})", curses.color_pair(6))
      else:
        self.sidebar.addstr(
          idx+4, 2, f"{timer[0]} ({timer[1]})", curses.color_pair(1))
